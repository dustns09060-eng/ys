...
async function pumasiApiGet(action, params = {}) {
  ...
  try {
    ...
    throw new Error(`API 응답을 읽지 못했습니다. (HTTP ${response.status})`);
  }

  if (!response.ok) {
    throw new Error(data?.message || data?.error || `API HTTP ${response.status}`);
  }

  if (!data?.ok) {
    const error = new Error(data?.message || data?.error || "품앗이 API 요청 실패");
...
function initPumasiStage1() {
  ...
  $("pumasiApiBtn")?.addEventListener("click", async () => {
    ...
    try {
      ...
      const auth = await pumasiApiGet("pumasiAuthStatus", {
        clientId: getPumasiClientId()
      });

      if (!auth.connected) {
        throw Object.assign(
          new Error("먼저 Instagram 계정을 연결해 주세요."),
      ...
      const completed = new Set(
        rawComments
          .map((comment) => pumasiNormalizeId(comment?.username))
          .filter(Boolean)
      );

      const uniqueWriters = new Set(completed);
      const check = getPumasiCheckTargets(participants);
      const instagramCommentCount = Number(data.instagramCommentCount || 0);
      const fetchedCommentCount = Number(data.commentCount || rawComments.length || 0);

      if (instagramCommentCount > 0 && fetchedCommentCount === 0) {
        throw Object.assign(
          new Error(
            `Instagram에는 댓글 ${instagramCommentCount}개가 있지만 ` +
            "Meta API에서 댓글 목록을 가져오지 못했습니다. " +
            "잘못된 누락자 판정을 막기 위해 검사를 중단했습니다."
          ),
          {
            code: "COMMENTS_API_EMPTY",
            details: data
          }
        );
      }
      ...
      renderPumasiResult(
        check.targets,
        completed,
        `Meta API · 댓글 작성자 ${uniqueWriters.size}명 / 댓글 ${fetchedCommentCount}개`,
        new Set(),
        check.selfId
      );

      status.textContent =
        `완료 · 댓글 작성자 ${uniqueWriters.size}명 확인 · ` +
        `댓글 ${fetchedCommentCount}개 · 검사 대상 ${check.targets.length}명` +
        (check.selfId ? ` (내 계정 @${check.selfId} 제외)` : "");
      ...
      if (code === "COMMENTS_API_EMPTY") {
        status.textContent = "⚠️ 댓글 조회 보류 · " + message;
        toast("댓글 수와 API 결과가 달라 판정을 중단했습니다.");
      } else if (code === "MEDIA_NOT_FOUND") {
        status.textContent = "게시물 확인 실패 · " + message;
        toast("게시물 링크를 확인해 주세요.");
      } else if (code === "INSTAGRAM_NOT_CONNECTED") {
        status.textContent = "Instagram 연결 필요 · " + message;
        toast("Instagram 계정을 먼저 연결해 주세요.");
      } else {
        status.textContent = "자동 확인 실패 · " + message;
        toast("댓글 자동 확인에 실패했습니다.");
      }

      console.error("[PUMASI V53]", {
        code,
        message,
